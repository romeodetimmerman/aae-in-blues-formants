...
def convert_to_wide_form():
    """
    convert the long-form dataset to wide form

    params
    ------
    none: reads the merged formant dataframe in data/interim/merged_formants_perceptive.csv

    returns
    -------
    none: saves the wide-form dataset to data/interim/formant_wide.csv
    """
    # get project root directory
    root_dir = Path(__file__).parent.parent.parent

    # load the long-form dataset
    input_path = root_dir / "data/interim/merged_formants_perceptive.csv"
    ...
    # save to csv
    output_path = root_dir / "data/interim/formant_wide.csv"
    df_wide.to_csv(output_path, index=False)
    print(f"saved wide-form data to {output_path}")


def main():
    convert_to_wide_form()
